  ...
  useEffect(() => {
    const fetchNutsData = async () => {
      try {
        ...
        console.log("CSV data received, processing...");
        ...
        if (geoJSON.features.length > 0) {
          ...
          if (sample.geometry.coordinates && sample.geometry.coordinates.length > 0) {
            const firstRing = sample.geometry.coordinates[0];
            if (firstRing && firstRing.length > 0) {
              const firstPoint = firstRing[0];
              console.log('First coordinate point:', firstPoint);
            }
          }
        }
